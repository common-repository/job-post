...
class Page_Template_Plugin {
	...
	private function __construct() {
		...
		add_filter('page_attributes_dropdown_pages_args', array( $this, 'rctjp_register_project_templates' ) );

		// Add a filter to the save post in order to inject out template into the page cache
		add_filter('wp_insert_post_data', array( $this, 'rctjp_register_project_templates' ) );

		// Add a filter to the template include in order to determine if the page has our template assigned and return it's path
		add_filter('template_include', array( $this, 'rctjp_view_project_template') );

		// Register hooks that are fired when the plugin is activated, deactivated, and uninstalled, respectively.
		register_deactivation_hook( __FILE__, array( $this, 'rctjp_deactivate' ) );

		// Add your templates to this array.
		$this->templates = array(
			'template-rct_job_post_applay_form.php' => __( 'RCT Job Post Application Form', $this->plugin_slug ),
			'template-rct_job_post_list.php' => __( 'RCT Job Post List', $this->plugin_slug )
	 ...
	 * @since	1.0.0
	 */
	public function rctjp_register_project_templates( $atts ) {
	 ...
	 * @since	1.0.0
	 */
	public function rctjp_view_project_template( $template ) {
	...
	*---------------------------------------------*/
	static function rctjp_deactivate( $network_wide ) {
		foreach($this as $value) {
			page-template-example::rctjp_delete_template( $value );
		}
	...
	*---------------------------------------------*/
	public function rctjp_delete_template( $filename ){				
